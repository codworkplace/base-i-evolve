...
class CaseSelector:
    ...
    async def select_case(self, competency_id: str, user_level: float = 0.5):
        ...
        if user_level < 0.4:
            ...
            suitable = [c for c in available_cases if c.get('difficulty', 'medium') == 'easy']
        elif user_level < 0.7:
            # Средний уровень
            suitable = [c for c in available_cases if c.get('difficulty', 'medium') in ['easy', 'medium']]
        else:
            # Высокий уровень - сложные кейсы
            suitable = [c for c in available_cases if c.get('difficulty', 'medium') == 'hard']
